    ...
    if (user) {
        ...
        if (member) {
            if (member.user === message.author) { //valida que no se kickee así mismo
                message.channel.send(`Ba-baka, no puedes kickearte a tí mismo ${member.user.username}-sempai `);
                return
            }
            if (member.user.username === client.user.username) { //valida que no intente kickear al bot
                message.channel.send(`Con que queriendo mandarme alv. Entrále prro :dagger: `);
                return
            }
            if (!member.kickable) {//valida que el bot tenga permisos para kickear al usuario
                message.channel.send(`jejeje Qué crees? No tengo permisos para kickiar a este vatillo`);
                return
            }
